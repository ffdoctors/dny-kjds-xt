...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        if not existing:
            ...
            admin = User(id=str(uuid.uuid4()), tenant_id=tenant.id, username="admin", 
                        password=pwd_context.hash("admin123"), email="admin@example.com", role="admin")
            db.add(admin)
            
            demo = User(id=str(uuid.uuid4()), tenant_id=tenant.id, username="demo",
                       password=pwd_context.hash("demo123"), email="demo@example.com", role="user")
            db.add(demo)
...
async def login(req: LoginRequest, db: Session = Depends(get_db)):
    ...
    token = jwt.encode({
        ...
        "tenantId": user.tenant_id, "tenantName": "默认租户", "plan": "enterprise"
    }, JWT_SECRET, algorithm=JWT_ALGORITHM)
    
    return {"access_token": token, "token_type": "bearer", "user": {
        "id": user.id, "username": user.username, "email": user.email,
        "role": user.role, "tenantId": user.tenant_id, "tenantName": "默认租户", "plan": "enterprise"
    }}
...
async def get_summary(period: str = "30", platform: str = "all", country: str = "all",
    ...
    return {
        ...
        "totalGsv": round(total_gmv * 1.05, 2),
        "totalRefund": round(sum(s.refund_amount for s in sales) if sales else 0, 2),
...
async def get_platform_stats(db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
    ...
    ).filter(SalesData.tenant_id == current_user["tenantId"]).group_by(SalesData.platform).all()
    return [{"platform": r[0], "gmv": float(r[1] or 0), "orders": r[2] or 0} for r in results]

@app.get("/api/dashboard/daily")
async def get_daily(period: str = "30", platform: str = "all", country: str = "all",
                  db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
    sales = db.query(SalesData).filter(SalesData.tenant_id == current_user["tenantId"]).order_by(SalesData.date.desc()).limit(50).all()
    return [{
        "id": s.id, "date": s.date, "platform": s.platform, "country": s.country,
        "product": s.product, "gmv": s.gmv, "gsv": s.gsv,
        "refund_amount": s.refund_amount, "refund_rate": s.refund_rate,
        "ad_cost": s.ad_cost, "roi": s.roi, "orders_count": s.orders_count
    } for s in sales]

@app.get("/api/sales")
async def get_sales(page: int = 1, limit: int = 50, platform: str = None, country: str = None,
                   db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
    query = db.query(SalesData).filter(SalesData.tenant_id == current_user["tenantId"])
    ...
    return {"data": [{
        "id": s.id, "date": s.date, "platform": s.platform, "country": s.country,
        "product": s.product, "gmv": s.gmv, "gsv": s.gsv,
        "refund_amount": s.refund_amount, "ad_cost": s.ad_cost, "roi": s.roi, "orders_count": s.orders_count
    } for s in sales], "total": total, "page": page, "limit": limit}
...
async def create_sale(data: SalesCreate, db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
    sale = SalesData(
        id=str(uuid.uuid4()), tenant_id=current_user["tenantId"],
        date=data.date, platform=data.platform, country=data.country, product=data.product,
        gmv=data.gmv, gsv=data.gsv, refund_amount=data.refund_amount,
        ad_cost=data.ad_cost, roi=data.roi, orders_count=data.orders_count
    )
    db.add(sale)
    db.commit()
...
async def export_sales(db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
    ...
    for i, s in enumerate(sales, 1):
        ...
        ws.write(i, 6, s.refund_amount); ws.write(i, 7, s.ad_cost)
        ws.write(i, 8, s.roi); ws.write(i, 9, s.orders_count)
    workbook.close()
    output.seek(0)
